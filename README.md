[![Build Status](https://travis-ci.com/camptocamp/meteotest_odoo.svg?token=3A3ZhwttEcmdqp7JzQb7&branch=master)](https://travis-ci.com/camptocamp/meteotest_odoo)

# Meteotest Odoo

**Our internal id for this project is: 1986.**

This project uses Docker.
Travis builds a new image for each change on the branches and for each new tag.

The images built on the master branch are built as `camptocamp/meteotest_odoo:latest`.
The images built on other branches are built as `camptocamp/meteotest_odoo:<branch-name>`.
The ones built from tags are built as `camptocamp/meteotest_odoo:<tag-name>`.
