...
* `docker-compose.yml`
 ...
 ```
 odoo:
  image: camptocamp/meteotest_odoo:10.0.0
   ...
   environment:
     ...
     - STATSD_CUSTOMER=meteotest_odoo
...
* `rancher.public.env`
 ...
 ```
 export DOMAIN_NAME=integration.meteotest.odoo.camptocamp.ch
 ...
 export AWS_BUCKETNAME=meteotest-odoo-integration
 
 export ODOO_SESSION_REDIS=1
 export ODOO_SESSION_REDIS_HOST=redis
 export ODOO_SESSION_REDIS_PREFIX=meteotest-odoo-integration
...
* `docker-compose.yml`
 ...
 letsencrypt:
   ...
   image: janeczku/rancher-letsencrypt:v0.3.0
   volumes:
     - meteotest-prod-certs:/etc/letsencrypt/production/certs
 ```
...
* `rancher.public.env`
 ...
 export AWS_BUCKETNAME=meteotest-odoo-prod
 
 export ODOO_SESSION_REDIS=1
 export ODOO_SESSION_REDIS_HOST=redis
 export ODOO_SESSION_REDIS_PREFIX=meteotest-odoo-prod
...
```
./rancher meteotest-odoo-integration up -d letsencrypt
./rancher meteotest-odoo-prod up -d letsencrypt
```
...
* Use the local `./rancher` script, example to start the containers:
  ...
  ```
  ./rancher meteotest-odoo-integration up -d
  ```
...
* Use the local `./rancher` script, example to upgrade the containers:
  ...
  ```
  ./rancher meteotest-odoo-integration up --pull --recreate --confirm-upgrade -d
  ```
